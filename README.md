...
An end-to-end RevOps portfolio project demonstrating data cleaning (ETL), financial modeling (Excel), operational process mapping (onboarding and billing), and leadership presentation (PowerPoint) using simulated SaaS CRM data.
...
2. **ETL & Financial Modeling (Excel)**: A data-cleaning pipeline that takes the raw CRM data and outputs a polished, formula-driven Excel sheet (`revenue_pipeline_tracker.xlsx`) containing dynamic weekly metrics, funnel velocity, and segment analytics.
...
4. **Leadership Deck (PowerPoint)**: A programmatically generated 16:9 widescreen PowerPoint deck summarizing findings and proposing the automation ROI for executive approval.
5. **Interactive Portfolio Web Page**: A beautiful, dark/light theme web dashboard (`index.html`) featuring interactive Chart.js visualizations, workflow tabs, an interactive slide deck viewer, and a download hub.
